...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		if writeUsageLogBestEffortForUser(ctx, s.cfg, user, s.usageLogRepo, usageLog, "service.openai_gateway") {
	...
	if billingErr != nil {
		...
		writeUsageLogBestEffortForUser(ctx, s.cfg, user, s.usageLogRepo, usageLog, "service.openai_gateway")
		return billingErr
	}
	writeUsageLogBestEffortForUser(ctx, s.cfg, user, s.usageLogRepo, usageLog, "service.openai_gateway")
